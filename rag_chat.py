...
class LocalSearchChain:
    ...
    def invoke(self, question: str, only_accounting: bool = False) -> str:
        top_docs = self.retriever.invoke(question, only_accounting=only_accounting)
...
def init_local_documents():
    corpus_path = Path("data/albo_download/documenti_corpus.jsonl")
...
def init_rag_system(embedding_models, llm_models, embed_batch_size: int, embed_pause_sec: float, build_if_missing=True):
    ...
    faiss_index_path = Path("data/albo_download/faiss_index")
    faiss_manifest_path = Path("data/albo_download/faiss_index_manifest.json")
    corpus_path = Path("data/albo_download/documenti_corpus.jsonl")
...
if user_question := st.chat_input("Chiedi qualcosa sui documenti (es. Quali ditte hanno avuto affidamenti diretti?)"):
    ...
    with st.spinner("Ricerca nei documenti e ragionamento in corso..."):
        try:
            response = rag_chain.invoke(user_question, only_accounting=only_accounting, only_personnel_competence=only_personnel)
