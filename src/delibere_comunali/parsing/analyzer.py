...
try:
    from extractor import DelibereExtractor
except ImportError:
...
def estrai_attori_procedimento(testo_atto: str) -> dict:
    ...
    if match:
        ...
        risultato["nome"] = match.group("nome").upper().strip()
...
def extract_from_pdf(path: Path, use_llm: bool = False, rf_model=None, text_dir: Path = None) -> dict:
    ...
    out["legal_urn"] = generate_legal_urn(out.get("doc_type"), out.get("data_atto"), out.get("numero_atto"))
    ...
    out["capitolo"] = adv_data.get("capitolo")
    ...
    out["responsabile"] = attori["nome"] if attori["nome"] != "NON IDENTIFICATO" else adv_data.get("responsabile")
...
def main():
    ...
    with metrics.start_operation("analisi_pdf") as op:
        ...
        for idx, pdf_file in enumerate(files):
            ...
            info["text_preview"] = text_full[:4000]
            corpus_rows.append({
                **info,
                "text": text_full,
            })
    ...
    for nome_gruppo, group_df in dfp.groupby("atto_group", dropna=False):
        ...
        record = {
            ...
            "rup_nome": next(iter(group_df["rup_nome"].dropna()), None),
            "rup_area": next(iter(group_df["rup_area"].dropna()), None),
            "rup_ruolo": next(iter(group_df["rup_ruolo"].dropna()), None),
