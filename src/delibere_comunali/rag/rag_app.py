    ...
    def __init__(self, retriever, prompt_template: str, llm_models):
        ...
        for model_name in _unique_non_empty(llm_models):
            try:
                ...
                elif model_name.startswith("mistral:") or "mistral" in model_name or "pixtral" in model_name:
                    if ChatMistralAI is None:
                        errs.append((model_name, "Libreria mancante per Mistral"))
                        continue
                    m_model = model_name.replace("mistral:", "")
                    llm = ChatMistralAI(model=m_model, temperature=0.1, api_key=os.environ.get("MISTRAL_API_KEY"))
                    self.llm_candidates.append((model_name, llm))
                else:
                    if ChatGoogleGenerativeAI is None:
                        errs.append((model_name, "Libreria mancante per Gemini"))
                        continue
...
def init_local_chain(base_dir: str):
    return _init_local_chain_core(Path(base_dir))
...
def init_local_documents(base_dir: str):
    return _init_local_documents_core(Path(base_dir))


@st.cache_resource(show_spinner="Inizializzazione Gemini con retriever locale...")
def init_gemini_local_retriever_chain(llm_models, base_dir: str):
    llm_models = tuple(llm_models)
    local_docs = _init_local_documents_core(Path(base_dir))
    if not local_docs:
        return None
...
def init_rag_system(embedding_models, llm_models, embed_batch_size: int, embed_pause_sec: float, build_if_missing=True, base_dir: str = "albo_download"):
    return _init_rag_system_core(tuple(embedding_models), tuple(llm_models), embed_batch_size, embed_pause_sec, build_if_missing, Path(base_dir))

def load_chat_history(base_dir: Path):
...
def save_chat_history(messages, base_dir: Path):
    chat_file = base_dir / "chat_history.json"
    chat_file.write_text(json.dumps(messages, ensure_ascii=False, indent=2), encoding="utf-8")

# --- INTEGRAZIONE CONTROL ROOM ---
_multi_tenant_rag_chains = {}

def esegui_query_rag_core(query: str, ente: str = "avella", only_accounting: bool = False) -> str:
    ...
    try:
        ...
        chain_key = f"{tenant_dir}_{only_accounting}"
        ...
        return _multi_tenant_rag_chains[chain_key].invoke(query, only_accounting=only_accounting)
...
def run_standalone_app():
    ...
    if hasattr(st, "query_params"):
        if "ente" in st.query_params:
            current_ente = st.query_params["ente"]
    ...
    if use_gemini:
        ...
        if not google_api_key:
            ...
            rag_chain = init_local_chain(str(APP_BASE_DIR))
        else:
            load_error = None
            ...
            if rag_chain is None:
                ...
                if use_local_retriever_with_gemini:
                    try:
                        rag_chain = init_gemini_local_retriever_chain(tuple(llm_models), str(APP_BASE_DIR))
                    except Exception as e:
    ...
    if rag_chain is None:
        st.error(
            f"Nessun corpus disponibile: file '{APP_BASE_DIR / 'documenti_corpus.jsonl'}' "
            "non trovato o vuoto."
        )
        st.stop()
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = load_chat_history(APP_BASE_DIR)

    if st.sidebar.button("🗑️ Cancella cronologia chat"):
        st.session_state.messages = []
        save_chat_history([], APP_BASE_DIR)
        st.rerun()
    ...
    if user_question := st.chat_input("Chiedi qualcosa sui documenti (es. Quali ditte hanno avuto affidamenti diretti?)"):
        ...
        save_chat_history(st.session_state.messages, APP_BASE_DIR)
