...
def main():
    ...
    if pd.notna(row.get('rup_nome')) and str(row['rup_nome']) != 'NON IDENTIFICATO':
        rup_name = str(row['rup_nome']).strip()
        area_name = str(row.get('rup_area', 'AREA NON SPECIFICATA')).strip()
        ...
        g.add((area_uri, RDF.type, COV.Organization)) # O un sottotipo più specifico
