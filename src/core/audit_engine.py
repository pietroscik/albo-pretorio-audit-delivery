...
def normalizza_beneficiario(nome: str) -> str:
    ...
    # Correzione specifica per refusi OCR ricorrenti
...
def normalizza_rup(testo_rup: str) -> str:
    """Normalizza il nome del Responsabile del Procedimento (RUP) in modo generalizzato."""
    ...
    formule_burocratiche = [
        ...
        "DECRETO", "FUNZIONI ATTRIBUITE", "AI SENSI"
    ]
    if any(formula in testo for formula in formule_burocratiche):
        return "NON IDENTIFICATO"
        
    # Pulizia generica: rimuove prefissi comuni
...
def main():
    ...
    # 1. Applica la normalizzazione preferendo il campo strutturato se presente
    df['beneficiario_norm'] = df['beneficiario'].apply(normalizza_beneficiario)
    
    if 'rup_nome' in df.columns:
        df['rup_norm'] = df['rup_nome'].fillna(df['responsabile']).apply(normalizza_rup)
    else:
    ...
    # Filtra solo gli atti validi per l'analisi
    ...
    if not df_analisi.empty:
        ...
        alerts.append("ℹ️ *Dati insufficienti per il rilevamento del frazionamento.*\n")
    ...
    if not affidamenti_diretti.empty:
        ...
        alerts.append("ℹ️ *Dati insufficienti per l'analisi della rotazione.*\n")
